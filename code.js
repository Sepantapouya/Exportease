...
class StringUtils {
    ...
    static tokenToDartVariable(name) {
        return name
            .replace(/[^a-zA-Z0-9]+/g, '_')
            .replace(/^[^a-zA-Z_]/, '_')
            .replace(/_{2,}/g, '_')
        ...
        for (const collection of tokenAnalysis.collections) {
            ...
            switch (format.toLowerCase()) {
                ...
                case 'sass':
                    ...
                    break;
                case 'json':
                    collectionFiles = formatGenerators_1.JSONGenerator.generateCollectionJSON(collection);
                    break;
                case 'js':
                case 'javascript':
                    ...
                    break;
                case 'dart':
                    collectionFiles = formatGenerators_1.DartGenerator.generateCollectionDart(collection);
                    break;
                case 'tailwind':
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.TailwindGenerator = exports.DartGenerator = exports.JSGenerator = exports.JSONGenerator = exports.SCSSGenerator = exports.CSSGenerator = exports.BaseGenerator = void 0;
const stringUtils_1 = __webpack_require__(820);
class BaseGenerator {
...
class JSONGenerator extends BaseGenerator {
    static generateCollectionJSON(collection) {
        const files = {};
        collection.modes.forEach(mode => {
            const fileName = `${stringUtils_1.StringUtils.slugify(collection.name)}-${stringUtils_1.StringUtils.slugify(mode.name)}.json`;
            const content = this.generateJSON(mode.tokens, collection.name, mode.name);
            files[fileName] = content;
        });
        return files;
    }
    static generateComprehensiveJSON(group) {
        const jsonObject = {
            _metadata: {
                generator: 'ExportEase Plugin',
                generated: new Date().toISOString()
    ...
    static generateJSON(tokens, collectionName, modeName) {
        const jsonObject = {
            _metadata: {
                generator: 'ExportEase Plugin',
                collection: collectionName,
...
class DartGenerator extends BaseGenerator {
    static generateCollectionDart(collection) {
        const files = {};
        collection.modes.forEach(mode => {
            const fileName = `${stringUtils_1.StringUtils.slugify(collection.name)}_${stringUtils_1.StringUtils.slugify(mode.name)}.dart`;
            const content = this.generateDart(mode.tokens, collection.name, mode.name);
            files[fileName] = content;
        });
        return files;
    }
    static generateComprehensiveDart(group) {
        ...
        group.collections.forEach(collection => {
            ...
            collection.modes.forEach(mode => {
                dart += `  // ${mode.name}\n`;
                mode.tokens.forEach(token => {
                    const dartVar = stringUtils_1.StringUtils.tokenToDartVariable(token.name);
    ...
    static generateDart(tokens, collectionName, modeName) {
        let dart = `// ${this.generateFileComment(collectionName, modeName)}\n\n`;
        const className = `${stringUtils_1.StringUtils.toPascalCase(collectionName)}${stringUtils_1.StringUtils.toPascalCase(modeName)}`;
    ...
    static formatDartValue(value, type) {
        switch (type) {
            case 'COLOR':
